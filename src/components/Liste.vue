<template>
  <ul class="container liste">
    ...
    <button v-on:click="suppFilm" class="btn btn-danger mt-2">
      Supprimer un fruits
    </button>
    <button v-on:click="suppTexte" class="btn btn-danger mt-2">
      Supprimer le texte
    </button>
    <button v-on:click="changeTitre" class="btn btn-danger mt-2">
      Modifier titre
    </button>
    <button v-on:click="changeTitreHeader" class="btn btn-danger mt-2">
      Modifier titre header
    </button>
    <h4>{{ soratra }}</h4>
    <p>
      prenom: {{ prenom }} et la taille est : {{ taille }} apres on reverse et
      donne {{ reversing() }}
    </p>
  </ul>
  <ul class="container liste">
